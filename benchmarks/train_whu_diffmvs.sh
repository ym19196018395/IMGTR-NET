...
# DiffMVS 源码根目录 (优先环境变量，自动兼容服务器多用户路径)
...
LOG_DIR="${LOG_DIR:-${PATCHMATCHNET_DIR}/checkpoints/diffmvs_whu_train}"
...
LOG_FILE="${PATCHMATCHNET_DIR}/txt_logs/train_whu_diffmvs_${timestamp}.log"
...
# 4. 执行训练
python "${SCRIPT_DIR}/train_whu_diffmvs.py" \
    --dataset dtu_whu \
...
echo "接下来请执行收官独立评测脚本: ./benchmarks/test_whu_diffmvs.sh ${LOG_DIR}/model_$(printf "%06d" $((EPOCHS - 1))).ckpt"
echo "=============================================================================="
